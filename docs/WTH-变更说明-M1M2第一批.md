...
# 终章 · PRD 执行状态总盘点（2026-09-05）

> 优化执行自 PRD v1.0 评审通过起，共 **14 个 commit**、八批次落地。
> 下表为 PRD 全部需求项的最终状态。
...
| ID | 需求 | 状态 | 交付 |
|---|---|---|---|
| S-01 | 清除硬编码 Key + 扫描门禁 | ✅ | commit f04313e + 桌面批；CI 常驻 |
| S-02 | README/文档对齐 | ✅ | a681539 |
| A-01 | 桌面端接内核 | ✅ 步骤1 | c7395b2 + 桌面批（ACP 桥接/开关回退/审批桥接；步骤2/3 见下） |
| A-02 | 子代理深度配置化 | ✅ | 370a208（默认 2，WTH_MAX_SUBAGENT_DEPTH；token 预算继承未做，见下） |
| A-03 | Windows 沙箱 | ✅ 两阶段 | 7e660e5（kill-on-close Job）+ 4ffb311（可选内存限额）；deny-ACL 见下 |
| F-01 | 本地模型一等公民 | ✅ 桌面侧 | ef3eb66（Ollama/vLLM 自动发现 + 免 Key；CLI 侧检测见下） |
| F-02 | 真 Token 计量 | ✅ | 511faef（BPE + CJK 感知；模型感知 API 就绪供逐点接入） |
| F-03 | 熔断接入采样 | ✅ | cba6a2e |
| F-04 | Git 一等公民工具 | ✅ | 370a208 |
| F-05 | 测试验证循环 | ✅ | 桌面批（test_cmd + 自动修复轮） |
| F-06 | 模型调度 | ✅ 三阶段 | ef3eb66（fallback 链/每模型计价）→ 4ffb311（子代理/工作流接入 + 摘要角色路由） |
| F-07 | MCP 服务器端 | ✅ 两阶段 | 64b6b89（stdio + 只读三件）→ 9ee2887（wth_ask 会话级工具）；HTTP 传输/写操作面见下 |
| F-08 | Web 搜索五引擎 | ✅ 桌面侧 | 桌面批（修复四引擎静默回退 bug）；CLI 侧 provider trait 见下 |
| F-09 | DAG 工作流增强 | ✅ 一阶段 | 桌面批（retry/timeout/output_limit）；嵌套/CLI 同引擎见下 |
| F-10 | 语义索引统一 | ❌ 未做 | 结构性大工程（CLI codebase-graph 与桌面检索的存储/服务合并），需独立工作流 |
| F-11 | ACP 反向 MCP 补全 | ❌ 未做（P2） | |
| F-12 | deploy_app 决断 | ✅ | 4ffb311（注册占位工具，修复预存测试失败） |
| U-01 | 桌面会话增强 | ✅ | 2b7c12e（fork 持久化缺陷修复；检索/重新生成分支已有） |
| U-02 | 用量与预算真数据 | ✅ | 桌面批（分价计费 + 预算拦截既有真实化确认） |
| U-03 | TUI 键位/i18n | ❌ 未做（P2） | |
| U-04 | 跨平台分发 | ❌ 未做（P2） | 需 macOS/Linux CI runner |
| U-05 | 浏览器工具 | ❌ 未做（P2） | |
| E-01 | 技能模板库 | ✅ | ef3eb66（10 技能 + 双端种子） |
| E-02 | 插件签名分发 | ❌ 未做（P2） | |
| Q-01 | 包名统一 | ✅ 全量 | 8e607ae（B0）+ 6ce1766（B1–B6，62 包）；基线 R9 关闭 |
| Q-02 | 测试补强 | ✅ 一阶段 | 桌面批 + 4ffb311（前端 15、Rust 51/79 局部） |
| Q-03 | 构建与体积优化 | ❌ 未做（P2） | |
| Q-04 | 贡献者体验包 | ✅ 部分 | 9ee2887（中文用户名环境文档）；ADR/依赖图未做 |

## 44. P0/P1/P2 完成度

- **P0（6/6）100%** —— S-01/S-02、A-01 步骤1、F-01、F-02、F-03、A-02、F-04、F-05、U-02 全部落地。
- **P1（10/11）≈91%** —— 唯余 F-10（语义索引统一），其依赖的存储格式合并属于跨 crate 结构改造，需要以独立工作流承接（涉及 CLI/GUI 两套既有索引的数据迁移，不宜在本会话仓促合并）。
- **P2 长尾** —— 已完成 Q-01 全量、F-12、A-03 二阶段、F-06 三阶段、F-07 二阶段、Q-04 部分；未做的 U-03/U-04/U-05/F-11/E-02/Q-03 均为 P2/P3 长尾，多项依赖本会话不具备的条件（跨平台 CI runner、签名证书、浏览器集成验证）。

## 45. 超出本环境边界的遗留（诚实清单）

1. **A-01 步骤 2/3**（桌面专属 IPC 全量切内核 ToolBridge、记忆切 xai-grok-memory、会话存储统一 JSONL、拆自研循环）+ **端到端联调**——需真实模型与 GUI 人工验证。
2. **F-10 语义索引统一**——结构性改造，独立工作流。
