...
export function EditorPanel({ theme }: { theme: "dark" | "light" }) {
  ...
  return (
    <div className="flex flex-col h-full min-h-0" style={{ background: "var(--surface-0)" }}>
      {/* 标签栏 */}
      <div className="flex items-center gap-1 px-2 pt-1.5 flex-shrink-0" style={{ background: "var(--surface-1)" }}>
        {openFiles.map((file) => (
          <div
            key={file.path}
            ...
            style={{
              ...
              borderBottom: file.path === activeFile ? "2px solid var(--accent-blue)" : "2px solid transparent",
            }}
          ...
          >
            ...
            {file.dirty && <span className="w-1.5 h-1.5 rounded-full" style={{ background: "var(--accent-yellow)" }} />}
            <button
              onClick={(e) => {
                e.stopPropagation();
      ...
      <div className="flex-1 min-h-0">
        {active ? (
          ...
          <div className="h-full flex flex-col items-center justify-center gap-2 text-xs" style={{ color: "var(--text-muted)" }}>
            <FileCode2 size={20} />
