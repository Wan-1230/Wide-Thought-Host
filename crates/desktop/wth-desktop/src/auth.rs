...
use crate::{
    credentials,
    settings::{GitHubProfile, persist_state_settings},
    state::AppState,
};
...
async fn fetch_profile(token: &str) -> Result<GitHubProfile, String> {
    ...
    let user: GitHubUserResponse = client
        ...
        .send()
        .await
        .map_err(|e| format!("读取 GitHub 资料失败：{e}"))?
        .error_for_status()
        .map_err(|e| format!("GitHub 资料请求失败：{e}"))?
        .json()
        .await
        .map_err(|e| format!("解析 GitHub 资料失败：{e}"))?;
    Ok(GitHubProfile {
        login: user.login,
        name: user.name,
...
pub async fn github_auth_poll(state: State<'_, AppState>) -> Result<GitHubAuthStatus, String> {
    let pending = state
        ...
        .map_err(|e| e.to_string())?
        .clone()
        .ok_or_else(|| "没有进行中的 GitHub 授权".to_string())?;
    if Instant::now() >= pending.expires_at {
        *state.github_auth.lock().map_err(|e| e.to_string())? = None;
        return Ok(GitHubAuthStatus {
            ...
            verification_uri: None,
            expires_in: None,
            message: Some("设备码已过期，请重新登录".into()),
        });
    }
    ...
    let state_name = match error.as_str() {
    ...
    Ok(GitHubAuthStatus {
        ...
        verification_uri: Some(pending.verification_uri),
        expires_in: Some(
            pending
                .expires_at
