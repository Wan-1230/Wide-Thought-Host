...
> **Note:** Wide Thought Host (WTH) applies personas through subagent resolution and roles, not through a `spawn_subagent` parameter. The main agent does not pass a persona name when it spawns a child.
...
When a persona applies, Wide Thought Host (WTH) resolves the effective model and reasoning effort in this order, highest priority first:
...
Wide Thought Host (WTH) manages worktrees through the `x.ai/git/worktree/*` extension methods, including an apply operation that merges changes back into the main working directory.
...
Wide Thought Host (WTH) shows running and finished work in side panes on the agent screen:
