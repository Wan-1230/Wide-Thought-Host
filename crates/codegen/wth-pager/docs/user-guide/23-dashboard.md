...
- **`wth dashboard`** — launches the TUI directly into the dashboard.
...
- **Ctrl+\\** — same as the slash command, two keystrokes. Configurable
  in `~/.wth/config.toml` under `[keybindings]` like every other shortcut.
...
```
 Wide Thought Host (WTH) · Dashboard — 4 agents · 2 awaiting
...
The state icon matches Wide Thought Host (WTH)'s sibling views (
`tasks_pane`):
...
process. The plan's Phase 4 ("supervisor / `wth --bg`") would list
