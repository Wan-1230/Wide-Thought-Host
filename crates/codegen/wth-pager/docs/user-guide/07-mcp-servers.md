...
MCP servers are configured in `~/.wth/config.toml` under `[mcp_servers.<name>]` sections.
...
Project-scoped files contribute `[mcp_servers]`, `[plugins]`, and `[permission]` entries. WTH reads most other config sections only from `~/.wth/config.toml`.
...
You must authenticate OAuth-based MCP servers before you can use them. Grok stores the resulting tokens under `~/.wth/mcp_credentials.json`. After you edit `config.toml`, press `r` in the `/mcps` modal to refresh the server list.
...
For stdio servers, Grok captures the process's standard error to `~/.wth/logs/mcp/<server>.stderr.log`, truncated on each launch. Check this file when a server starts but fails to handshake:

```bash
tail -f ~/.wth/logs/mcp/filesystem.stderr.log
```

### Viewing Server Status

Use `wth inspect` to see all loaded MCP servers and their sources:
