...
```bash
# Run with workspace sandbox (read everywhere, write to CWD + temp dirs + ~/.wth/)
grok --sandbox workspace

# Read-only mode (read everywhere, write only to ~/.wth/ + temp dirs)
grok --sandbox read-only

# Most restrictive profile (read CWD + system paths, write CWD + temp dirs + ~/.wth/, no child network)
grok --sandbox strict
```
...
| `workspace`           | Everywhere         | CWD + `~/.wth/` + `/tmp` + `/var/tmp`         | Allowed       | Normal development                |
| `devbox`              | Everywhere         | All top-level dirs except `/data`              | Allowed       | Disposable dev VMs                |
| `read-only`           | Everywhere         | `~/.wth/` + `/tmp` + `/var/tmp`               | Blocked¹      | Exploration, code review          |
| `strict`              | CWD + system paths | CWD + `~/.wth/` + `/tmp` + `/var/tmp`         | Blocked¹      | Untrusted code                    |
...
**workspace** -- The recommended profile for everyday development. The agent can read any file on the system (for understanding dependencies, system libraries, etc.) but can only write to the current working directory, `~/.wth/`, and temp directories (`/tmp`, `/var/tmp`, plus the macOS temp dirs). Network access is allowed for tools like `web_search` and MCP servers.
...
**read-only** -- Use when you want the agent to analyze code without modifying your project files. The agent can read everything but can only write to `~/.wth/` (needed for session persistence) and temp directories. Child-process network access is blocked on Linux (no-op on macOS).

**strict** -- The most restrictive profile, for reviewing untrusted code. The agent can only read files within the current working directory and essential system paths. Writes are limited to CWD, `~/.wth/`, and temp directories. Child-process network access is blocked on Linux (no-op on macOS).
...
for the life of the session**. When you resume it (`wth --resume <id>`,
`wth --continue`, or `wth -r`), Grok restores that same profile automatically —
...
Sandbox events are logged to `~/.wth/sandbox-events.jsonl` for debugging. Events include:
