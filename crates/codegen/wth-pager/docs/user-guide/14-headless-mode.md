...
By default, each `wth -p` invocation creates a fresh session. To maintain context across calls, use session flags.
...
- **`wth login --device-auth`** (or `--device-code`) — no browser needed on the target machine.
  See [Authentication > Device Code Flow](02-authentication.md#device-code-flow).
- **`wth login`** — browser-based OAuth2 on machines with a GUI.
...
- Combine `--yolo` with `--rules` to set guardrails: `wth -p "..." --yolo --rules "Never delete files"`.
...
For containers or CI, mount `~/.wth` read-only:
...
- Resume: `wth -p "continue" --resume "<id>"` or `wth -p "continue" --continue`
