...
2. Create a hook file, e.g. `~/.wth/hooks/session-start.json`:
...
The Claude and Cursor hook sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] hooks = false` in `~/.wth/config.toml` or the corresponding environment variable. See [Configuration](05-configuration.md#harness-compatibility) for details.
...
- Global hooks (`~/.wth/hooks/`) run with your user permissions -- treat them like shell scripts.
