//! Headless single-turn mode (`wth -p "prompt"`).
//!
...
fn auth_required_message(interactive: bool) -> String {
    if interactive {
        "Not signed in. Run `wth login` to authenticate \
         (or `wth login --device-code` if no browser is available)."
            .to_string()
    } else {
...
pub async fn run_single_turn(
    ...
    // hub_url lives on `wth workspace` / WorkspaceStartArgs only).
