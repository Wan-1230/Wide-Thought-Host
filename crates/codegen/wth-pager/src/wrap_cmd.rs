//! `wth wrap` — run any command in a local PTY that forwards its clipboard.
//!
//! Generalizes the `wth ssh` wrapper: spawns an arbitrary command inside a
...
//! run — a single shell-quoted string (`wth wrap "mycli ssh host"`) or a shell
...
/// The program and argv `wth wrap` will actually spawn.
#[derive(Clone)]
struct SpawnPlan {
    program: String,
    args: Vec<String>,
...
fn derive_spawn(
    ...
    // (`wth wrap "mycli ssh host"`), not a program name: hand it to the shell
    ...
    // name, and an empty one (`wth wrap "$PROG" ...` with `$PROG` unset)
...
///
/// Unlike `wth ssh`, `wth wrap` does not gate on the terminal brand: the user
