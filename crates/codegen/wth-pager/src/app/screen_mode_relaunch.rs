...
/// `wth "do the thing"` does not re-submit on resume. Keeps everything else
...
pub(crate) fn build_screen_mode_relaunch_args(
    ...
    while let Some(arg) = iter.next() {
        ...
        // Bare positional prompt (e.g. `wth "fix the bug"`). Must not re-fire
...
/// as a successful `exec` — plain `wth --resume <id>` (or even
/// `wth --minimal --resume <id>`) is not enough after `/fullscreen` when
...
mod tests {
    ...
    fn double_dash_and_following_positionals_dropped() {
        // `wth --no-leader -- "fix the bug"`: everything after `--` is the
    ...
    fn resume_without_value_then_flag_is_not_eaten() {
        // `wth --resume --no-leader` (resume most-recent; next token is a flag).
        let out = build_screen_mode_relaunch_args(
