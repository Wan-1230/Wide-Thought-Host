...
async fn install_npm_calls_npm_with_version_arg() {
    ...
    // WTH Build privacy fork installs the community package name.
    assert!(args.contains("@wth-build/wth@0.1.181"), "args: {args}");
}
...
async fn install_npm_falls_back_to_dist_tag_on_no_target() {
    ...
    let log = g.args_log();
    assert!(
        log[0].contains("@wth-build/wth@latest"),
...
async fn install_npm_falls_back_to_alpha_dist_tag_on_alpha_channel() {
    ...
    let log = g.args_log();
    assert!(
        log[0].contains("@wth-build/wth@alpha"),
