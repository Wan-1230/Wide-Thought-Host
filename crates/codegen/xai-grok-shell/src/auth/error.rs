...
#[non_exhaustive]
pub enum AuthError {
    #[error("Not logged in. Run `wth login`.")]
    NotLoggedIn,

    /// Token expired and no refresh authority available.
    #[error("Token expired. Run `wth login` to re-authenticate.")]
    TokenExpiredNoRefresh,

    /// Server rejected the token (401) with no recovery path.
    #[error("Authentication rejected by server. Run `wth login` to re-authenticate.")]
    ...
    #[error("{message} Run `wth login` to sign in with the required team.")]
    PinnedTeamMismatch { message: String },

    /// Cached API-key session rejected because API-key auth is disabled.
    #[error("API-key auth is disabled by your administrator. Run `wth login` to authenticate.")]
    ApiKeyAuthDisabled,
    ...
    pub(crate) fn user_message(self) -> &'static str {
        match self {
            Self::RefreshTokenRejected => {
                "Your session has expired. Run `wth login` to sign in again."
            }
            Self::ClientRejected => {
                "Authentication is temporarily unavailable. Run `wth login` if this persists."
            }
            Self::Other => {
                "Authentication could not be refreshed. Run `wth login` to sign in again."
            }
        }
    }
}
