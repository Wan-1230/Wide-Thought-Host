...
pub async fn run_stdio_agent(
    ...
    // Log the client that launched us (set by grok-desktop when spawning `wth agent stdio`).
    ...
    let result = local_set
        .run_until(async move {
            ...
            // `wth agent stdio` is a local/interactive entrypoint (spawned by
...
async fn run_headless_inner(
    ...
    register_fs_watch_runtime();
    xai_grok_telemetry::unified_log::set_version(xai_grok_version::VERSION);
    // `wth agent [headless]` serves non-TUI automation; stamp proxy requests
    // as headless. IDE-facing `wth agent stdio` stays interactive.
    ...
    const HEADLESS_NO_SESSION: &str = "Headless mode requires a grok.com session. \
        Run `wth login` to sign in, or use `wth agent stdio` for API-key access.";
    ...
    let (mut auth, did_browser_flow) = if no_browser {
        ...
        match auth_manager.current() {
            ...
            None if auth_manager.is_expired() => {
                anyhow::bail!("Session expired. Please run 'wth login' to re-authenticate.")
            }
            None => anyhow::bail!("No cached credentials found. Run `wth login`."),
        }
...
///
/// * `relay_on_demand == false` (default — explicit `wth agent leader`
...
pub async fn run_leader(
    ...
    local_set
        .run_until(async move {
            ...
            // `wth agent leader` (devbox / systemd) has no local IPC clients
...
mod tests {
    ...
    /// `wth agent leader` (devbox/systemd — no local IPC clients,
        ...
        local
            .run_until(async {
                let slot = spawn_leader_relay(
                    config,
                    false, // eager: explicit `wth agent leader` invocation
                    demand_rx,
                    ws_to_agent_tx,
                    agent_to_ws_tx.clone(),
                    cancel.clone(),
