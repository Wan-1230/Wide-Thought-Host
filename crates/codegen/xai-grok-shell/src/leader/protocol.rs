...
pub struct LeaderCapabilities {
    ...
    /// (driven by `wth update`). Old leaders default to `false`, so a new
...
pub enum ControlCommand {
    ...
    /// `wth update`). The leader stops admitting new turns, waits a bounded
    ...
    ///
    /// `to_version` is the version `wth update` just installed; the leader uses
