...
/// Maximum reconnection attempts for bounded mode (headless/`wth -p`).
...
pub enum ReconnectPolicy {
    ...
    /// Suitable for headless/`wth -p` where hanging forever is unacceptable.
...
/// auto-update or `wth update` atomically swaps that symlink, `current_exe()`
